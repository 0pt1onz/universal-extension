...
const SKYSHOWTIME_URL = /^https?:\/\/(www\.)?skyshowtime\.com\/[a-z]{2}\/(stream|watch)\/(tv|movie)\/[a-z0-9-]+/i
...
function parseSeasonEpisodeFromBody(bodyText: string): { season: number | null; episode: number | null } {
    ...
    if (sE1) return { season: parseInt(sE1[1], 10), episode: parseInt(sE1[2], 10) }

    const sE2 = bodyText.match(/(\d+)x(\d+)/i)
    if (sE2) return { season: parseInt(sE2[1], 10), episode: parseInt(sE2[2], 10) }
