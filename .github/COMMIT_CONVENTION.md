...
> This is adapted from [Angular's commit convention](https://github.com/conventional-changelog/conventional-changelog/tree/master/packages/conventional-changelog-angular) and from [PreMiD's commit convention](https://github.com/PreMiD/Activities/blob/main/.github/COMMIT_CONVENTION.md).

### TL;DR:

Messages must be following the regex:

```js
/^(revert: )?(feat|fix|refactor|perf|test|workflow|chore)(\(.+\))?: .{1,72}/;
```
...
Other prefixes are up to your discretion. Suggested prefixes are `chore`, `refactor`, and `test` for non-changelog related tasks.
